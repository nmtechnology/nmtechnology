<template>
  <div :class="containerClasses">
    ...
    <div v-if="variant === 'landing'" class="flex flex-col items-start -ml-3 sm:-ml-5">
      ...
      <span v-if="showSubtitle" class="text-xs sm:text-sm text-green-400 font-semibold tracking-wider uppercase ml-0.5">
...
const props = defineProps({
  variant: {
    type: String,
    default: 'header',
    validator: (value) => ['header', 'service', 'mobile', 'footer', 'landing', 'modal'].includes(value)
  },
  size: {
    type: String,
    default: 'medium',
...
const containerClasses = computed(() => {
  ...
  const variantClasses = {
    ...
    service: [
      'justify-center', 'mb-8', 'sm:mb-12', 'bg-gray-800/30', 'backdrop-blur-sm', 
      'rounded-xl', 'sm:rounded-2xl', 'border', 'border-green-500/20', 'py-3', 'sm:py-4', 'px-4', 'sm:px-6', 
      'w-fit', 'mx-auto', 'shadow-lg', 'group'
    ],
...
const logoClasses = computed(() => {
  const sizeClasses = {
    small: ['h-6', 'sm:h-8', 'w-auto'],
    medium: ['h-8', 'sm:h-10', 'w-auto'],
    large: ['h-10', 'sm:h-12', 'w-auto'],
    xl: ['h-16', 'sm:h-20', 'w-auto']
  }
  
  const variantClasses = {
    header: ['mr-1', 'sm:mr-2', 'transition-transform', 'duration-300', 'group-hover:scale-105'],
...
const textClasses = computed(() => {
  ...
  const variantClasses = {
    header: [
      'italic', 'font-extrabold', 'text-white', '-ml-3', 'sm:-ml-5', 
      'transition-colors', 'duration-300', 'group-hover:text-green-400',
      'whitespace-nowrap'
    ],
    service: [
      'italic', 'font-extrabold', 'text-white', '-ml-2', 'sm:-ml-3', 
      'transition-colors', 'duration-300', 'group-hover:text-green-400'
    ],
    mobile: [
      'italic', 'font-extrabold', 'text-white', '-ml-1',
      'whitespace-nowrap'
    ],
    footer: [
      'italic', 'font-extrabold', 'text-white', '-ml-3', 'sm:-ml-5',
    ...
    landing: [
      'italic', 'font-extrabold', 'text-white', 'transition-colors', 'duration-300'
    ],
    modal: [
