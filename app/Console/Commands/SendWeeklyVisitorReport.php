<?php

namespace App\Console\Commands;

use Illuminate\Console\Command;
use App\Services\VisitorReportService;

class SendWeeklyVisitorReport extends Command
{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'visitor:send-weekly-report';

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Send weekly visitor statistics report with visual SVG graphs';

    /**
     * Execute the console command.
     */
    public function handle()
    {
        $this->info('Generating weekly visitor report with visual graphs...');
        
        $success = VisitorReportService::sendWeeklyReport();
        
        if ($success) {
            $this->info('✅ Weekly visitor report sent successfully.');
            return 0;
        } else {
            $this->error('❌ Failed to send weekly visitor report. Check logs.');
            return 1;
        }
    }
}
